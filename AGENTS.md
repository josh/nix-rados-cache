...
Requires `ceph-mon`, `ceph-osd`, `librados-dev`, and `nix` with a running daemon. The harness starts a throwaway Ceph cluster; the first startup attempt is occasionally flaky and is retried automatically. `nix-ca-derivations.txtar` is skipped unless the daemon has `ca-derivations` enabled (`extra-experimental-features = ca-derivations` in `/etc/nix/nix.custom.conf`, then restart `nix-daemon`).
